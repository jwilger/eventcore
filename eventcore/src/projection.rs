    ...
    C: CheckpointStore,
{
        ...
        R::Error: std::fmt::Display,
    {
        ...
        loop {
            match current_step {
                ProjectionStep::Yield(ProjectionEffect::LoadCheckpoint { ref name }) => {
                    let result = match &self.checkpoint_store {
                        Some(cs) => cs.load(name).await.map_err(|e| e.to_string()),
                        None => Ok(None),
                    };
                    current_step =
                        pipeline.resume(ProjectionEffectResult::CheckpointLoaded(result));
                }
                ProjectionStep::Yield(ProjectionEffect::ReadEvents { filter, page }) => {
                    let result = self
                        .store
                        .read_events::<P::Event>(filter, page)
                        .await
                        .map_err(|e| e.to_string());
                    current_step = pipeline.resume(ProjectionEffectResult::EventsRead(result));
                }
                ProjectionStep::Yield(ProjectionEffect::SaveCheckpoint { ref name, position }) => {
                    let result = match &self.checkpoint_store {
                        Some(cs) => cs.save(name, position).await.map_err(|e| e.to_string()),
                        None => Ok(()),
                    };
                    current_step = pipeline.resume(ProjectionEffectResult::CheckpointSaved(result));
                }
                ProjectionStep::Yield(ProjectionEffect::Sleep { duration }) => {
                    tokio::time::sleep(duration).await;
                    current_step = pipeline.resume(ProjectionEffectResult::Slept);
                }
