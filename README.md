...
```rust
// Command and Event macros are re-exported from eventcore (enabled by default)
use eventcore::{Command, Event, require, StreamId};
...
|---------|---------|-------------|
| `macros` | Yes | Re-exports `#[derive(Command)]` and `#[derive(Event)]` from `eventcore-macros` |
